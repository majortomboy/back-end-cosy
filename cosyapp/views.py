...
from .serializers import PartSerializer, ProjectSerializer, TaskSerializer, ToBuyItemSerializer, RegisterUserSerializer, MyTokenObtainPairSerializer, ReferencePhotoSerializer
from rest_framework import viewsets, status, generics, permissions, status, response
from .models import Project, Part, ReferencePhoto, Task, ToBuyItem
...
'''
Reference photo serializers
'''
class ProjectReferencePhotoList(generics.ListCreateAPIView):
    permission_classes = [AllowAny]
    serializer_class = ReferencePhotoSerializer

    def get_queryset(self):
        return ReferencePhoto.objects.filter(project=self.kwargs.get('pk')).order_by('id')

class ReferencePhotoList(APIView):
    permission_classes = [AllowAny]
    '''
    List all projects, or create a new project
    '''
    def get(self, request, format=None):
        photos = ReferencePhoto.objects.all().order_by('id')
        serializer = ReferencePhotoSerializer(photos, many=True)
        return Response(serializer.data)

    def post(self, request, format=None):
        serializer = ReferencePhotoSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
class ReferencePhotoDetail(APIView):
    permission_classes = [AllowAny]
    """
    Retrieve, update, or delete a task instance.
    """
    def get_photo(self, pk):
        try:
            return ReferencePhoto.objects.get(pk=pk)
        except ReferencePhoto.DoesNotExist:
            raise Http404

    def get(self, request, pk, format=None):
        photo = self.get_photo(pk).order_by('id')
        serializer = ReferencePhotoSerializer(photo)
        return Response(serializer.data)

    def put(self, request, pk, format=None):
        photo = self.get_photo(pk)
        serializer = ReferencePhotoSerializer(photo, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def patch(self, request, pk, format=None):
        photo = self.get_photo(pk)
        serializer = ReferencePhotoSerializer(photo, data=request.data, partial=True)
